...
# ─── 交互配置 ────────────────────────────────────────────
read -rp "管理后台端口 [默认 $DEFAULT_UI_PORT]: " UI_PORT
UI_PORT=${UI_PORT:-$DEFAULT_UI_PORT}

read -rp "后端 API 端口 [默认 $DEFAULT_API_PORT]: " API_PORT
...
echo ""
info "管理后台将运行在端口: ${BOLD}$UI_PORT${NC}"
info "后端 API 运行在端口:  ${BOLD}$API_PORT${NC}"
echo ""
...
pkg_install curl wget git unzip ca-certificates gnupg lsb-release 2>/dev/null || true
ok "基础工具就绪"
...
if command -v go &>/dev/null; then
    ...
    if awk "BEGIN{exit !($cur >= $GO_MIN_VERSION)}"; then
        ok "Go $cur 已安装，跳过"
        need_go=false
    else
...
if ! command -v nginx &>/dev/null; then
    ...
    if [ "$PKG_MGR" = "apt" ]; then
        ...
        pkg_install epel-release nginx
    fi
    ok "nginx 安装完成"
else
...
if nginx -V 2>&1 | grep -q 'with-stream'; then
    ...
    if [ "$PKG_MGR" = "apt" ]; then
        ...
        pkg_install nginx-extras 2>/dev/null || \
...
cd "$TMP_DIR/backend"
info "go build..."
GOFLAGS="-mod=mod" go build -ldflags="-s -w" -o nginxflow-server . 2>&1
ok "后端编译完成"
...
# ─── 部署文件 ────────────────────────────────────────────
step "部署文件到 $INSTALL_DIR"
mkdir -p "$INSTALL_DIR/data"
cp "$TMP_DIR/backend/nginxflow-server" "$INSTALL_DIR/"
cp -r "$TMP_DIR/frontend/dist" "$INSTALL_DIR/frontend"
...
if [ -f "$NGINX_CONF" ] && ! grep -q 'nginxflow' "$NGINX_CONF"; then
    cp "$NGINX_CONF" "${NGINX_CONF}.bak.$(date +%Y%m%d%H%M%S)"
    info "已备份原 nginx.conf"
fi
...
nginx -t && ok "nginx 语法检查通过" || err "nginx 配置语法错误，请检查"
...
echo -e "${GREEN}${BOLD}╚═══════════════════════════════════════════╝${NC}"
echo ""
echo -e "  管理后台:  ${BOLD}http://${IP}:${UI_PORT}${NC}"
